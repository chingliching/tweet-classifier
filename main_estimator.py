...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 26 18:39:17 2018
...
def crossValidate(num_hidden, dropout, training_steps=10, batch_size=93):
    
    fold=0 #initialize
    embed_size=20 #result from BOW_embedding
    full, vocab_dict, embedding_matrix = segment('train.csv',size=embed_size)
    train,valid = split(full,fold)
    batch_size=93 #change later
    steps = 10 #change later
    
    feature_columns = [tf.feature_column.numeric_column("x", shape=(batch_size,))]
    ...
    estimator = tf.contrib.learn.DynamicRnnEstimator(
            ...
            dropout_keep_probabilities=None, #add dropout later
            model_dir=None,
    ...
    input_fn = tf.estimator.inputs.numpy_input_fn(
            {"x": train.data}, train.target, batch_size=len(train.target), num_epochs=10, shuffle=True)
    eval_input_fn = tf.estimator.inputs.numpy_input_fn(
            {"x": valid.data}, valid.target, batch_size=len(valid.target), num_epochs=steps, shuffle=False)

    estimator.fit(input_fn=input_fn, steps=steps)
    eval_metricsestimator.evaluate(input_fn=eval_input_fn)
