...
def train():
    ...
    model.add(LSTM(rs[0], return_sequences=True, input_shape=(X_train.shape[1], X_train.shape[2])))
    for r in rs[1:-1]:
    ...
    history = model.fit(X_train, y_train, validation_split=0.2, epochs=20, batch_size=64, shuffle=True, verbose=1)
